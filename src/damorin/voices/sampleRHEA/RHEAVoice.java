package damorin.voices.sampleRHEA;
...
public class RHEAVoice implements Voice {
     ...
     */
    public RHEAVoice(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
    ...
    public Opinion solve(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        opinion.setAction(population[0].actions[0]);
        opinion.setActionValue(evaluate(population[0], heuristic, stateObs));
        return opinion;
    }
    ...
    private void runIteration(StateObservation stateObs) {
        ...
        if (NUM_INDIVIDUALS > 1) {
            ...
            Individual newind = new Individual(SIMULATION_DEPTH, Agent.getNumActions(), randomGenerator).mutate(MUTATION);
    ...
    private Individual crossover() {
        ...
        if (NUM_INDIVIDUALS > 1) {
            newind = new Individual(SIMULATION_DEPTH, Agent.getNumActions(), randomGenerator);
    ...
    private void init_pop(StateObservation stateObs) {
        ...
        for (int i = 0; i < POPULATION_SIZE; i++) {
            if (i == 0 || remaining > avgTimeTakenEval && remaining > BREAK_MS) {
                population[i] = new Individual(SIMULATION_DEPTH, Agent.getNumActions(), randomGenerator);
                evaluate(population[i], heuristic, stateObs);
                remaining = timer.remainingTimeMillis();
                NUM_INDIVIDUALS = i + 1;
            } else {
