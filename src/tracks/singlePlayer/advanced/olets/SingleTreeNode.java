...
//                    MCTSVoice.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + SingleMCTSPlayer.epsilon)) +
...
//        }
//        _currentObservation.advance(MCTSVoice.actions[selectedIdx]);
//        return selected;
//    }
...
//
//                SingleTreeNode child = children[i];
//                double hvVal = child.expectimax + MCTSVoice.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + SingleMCTSPlayer.epsilon)) + SingleMCTSPlayer.randomGenerator.nextDouble() * SingleMCTSPlayer.epsilon - this.tabooBias;
//
//                // small sampleRandom numbers: break ties in unexpanded nodes
//                if (hvVal > bestValue) {
//                    selected = child;
...
//        return totValue / nVisits + MCTSVoice.K * Math.sqrt(Math.log(parent.nVisits + 1) / (nVisits + SingleMCTSPlayer.epsilon)) - tabooBias;
//    }

//    private double getEmaxScore() {
//        return expectimax + MCTSVoice.K * Math.sqrt(Math.log(parent.nVisits + 1) / (nVisits + SingleMCTSPlayer.epsilon)) - tabooBias;
//    }
